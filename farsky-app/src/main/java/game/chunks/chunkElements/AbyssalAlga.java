...
public final class AbyssalAlga extends ChunkElement {
   ...
   public AbyssalAlga(Point pos) {
      this.position = new Point(0.0F, 0.0F, 0.0F);
      this.position.set(pos);
      this.yRotation = (float)(Math.random() * 180.0);
      this.scale = 15.0F + (float)(Math.random() * 10.0);
      this.brightness = (int)((float)(Math.random() + 0.5) * 10.0F) / 10.0F;
   }

   public static void loadAssets() {
      vbo = ModelLoader.loadMesh("abyssalAlga");
      textureId = ModelLoader.loadTexture("abyssalAlga");
   }
   ...
   public static void beginBatch() {
      ...
      Shaders.setUniform("amplitude", 3.5);
